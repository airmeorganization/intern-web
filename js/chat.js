...
export async function loadChatList(currentUserId) {
  ...
  const { data: messages, error } = await supabase
    .from("messages")
    .select(
      "*, sender:profiles!messages_sender_id_fkey(id, full_name, username), receiver:profiles!messages_receiver_id_fkey(id, full_name, username)",
    )
  ...
  if (error) {
    container.innerHTML =
      '<div class="p-4 text-center text-red-500">Error loading chats</div>';
    return;
  }
